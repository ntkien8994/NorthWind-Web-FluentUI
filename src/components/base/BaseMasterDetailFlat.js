...
import GisOverlay from '../controls/GisOverlay';

class BaseMasterDetailFlat extends BaseComponent {
    constructor(props) {
        super(props);
    ...
    getToolbars = () => {
        ...
        return ([
            { commandName: Constant.commandName.close, value: "Hủy bỏ" },
            { commandName: Constant.commandName.saveAndNew, value: "Lưu & Thêm mới" },
            { commandName: Constant.commandName.save, value: "Lưu", primary: true, type: 'submit' },
    ...
    //description: overide lại để vẽ form
    //-----------------------------------
    //created by: ntkien 
    //created date: 05.10.2020
    getDetailForm(props,errors, touched) {
    ...
    render() {
        ...
        let configToolbars = me.getToolbars();
        configToolbars = configToolbars ?? [];
        let overFlowToolBar = me.getOverFlowToolbars();
        return (
            <>
                <Formik
                    initialValues={{
                        CustomerCode:'kh001',
                        CustomerName:'Nguyễn Văn Têng',
                        BirthDate: undefined,
                        BankId:3
                    }}
                    ...
                    validationSchema={me.getValidateForm()}
                >
                    {(props,errors, touched) => (
                        <form onSubmit={props.handleSubmit}>
                            <div className='detail-flat-toolbar' >
                                <div className='detail-flat-toolbar-left'>
                                    <GisBreadcrumb items={itemsBreadcrumb} />
                                </div>
                                <div className='detail-flat-toolbar-right'>
                                    {
                                        configToolbars.map(
                                            item => (<DefaultButton
                                                type={item.type}
                                                iconProps={item.iconProps}
                                                className={item.primary ? '' : 'default-btn'}
                                                text={item.value}
                                                primary={item.primary}
                                            //onClick={() => { me.toolbar_Click(item.commandName) }}
                                            />)
                                        )
                                    }
                                </div>
                            </div>
                            <div className='detail-flat-form' id='containerElement'>
                                {
                                    me.getDetailForm(props)

                                }
                                {
                                    me.props.showOverLay && <GisOverlay label='Đang thực hiện...' />
                                }
                            </div>
                        </form>
                    )}
...
export default BaseMasterDetailFlat